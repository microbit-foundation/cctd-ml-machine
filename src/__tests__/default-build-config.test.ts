...
describe('Default build config test', () => {
  ...
  test('Default config should be unbranded', () => {
    const windiContent = fs.readFileSync(
      'windi.config.js',
    );
    ...
    const message = "The default config file was determined to be a branded version of ML machine. Please run command 'node prepEnv.js unbranded' before committing"
