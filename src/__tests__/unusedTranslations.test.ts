...
test("All translations should be used", async () => {
	...
	for (let i = 0; i < translationKeys.length; i++) {
		...
		expect(filesIncludesExpression(flatten, translationKey), "unused translation --> '" + translationKey + "' \n confirm with command .. \n grep -rnw ./src -e '" + translationKey + "'").toEqual(true);
	}
}, 60000 * 10);