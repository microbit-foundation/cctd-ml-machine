...
import { createFeatureToken, type FeatureToken } from "../../../core/featureprovider/FeatureToken";
...
export const featureList: { [K in JSONFeatureKeys]: FeatureToken<Features[K]> } =
    _featureNames.reduce((acc, k, i) => {
        (acc as any)[k] = createFeatureToken<JSONFeatureKeys, Features[typeof k]>(String(k) as JSONFeatureKeys)
        return acc;
