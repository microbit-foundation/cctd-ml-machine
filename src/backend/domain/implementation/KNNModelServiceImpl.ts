...
export class KNNModelServiceImpl implements KNNModelService {
    ...
    getPoints(): LabelledPoint[] {
        ...
        const labels = this.dataService.getTrainingDataset(settings.shouldNormalize()).getLabels();
        const features = this.dataService.getTrainingDataset(settings.shouldNormalize()).getFeatureSet();
