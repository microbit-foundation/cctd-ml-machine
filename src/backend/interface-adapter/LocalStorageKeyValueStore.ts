...
export default class LocalStorageKeyValueStore<T> implements KeyValueStore<T> {
    ...
    constructor(options?: { prefix?: string; keySerializer?: (key: T) => string }) {
        ...
        this.keySerializer =
            options?.keySerializer ?? ((k: T) => (typeof k === "string" ? (k as unknown as string) : JSON.stringify(k)));
    ...
    getValue<U>(key: T): U | undefined {
        ...
        const raw = fs instanceof Map ? fs.get(this.fullKey(key)) : fs.getItem(this.fullKey(key));
        if (raw == null) {
            return undefined;
        }
