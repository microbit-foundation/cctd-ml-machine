...
export class KNNModelPendingSettingsListener implements ModelTrainingListener {
    ...
    setDependencies(modelService: ModelService, classifierService: ClassifierService): void {
    ...
    async onModelTrainingChanged(modelTraining: ModelTraining): Promise<void> {
        ...
        if (!this.modelService || !this.classifierService) {
            this.log.warn("ModelService is not set in KNNModelPendingSettingsListener. Cannot handle model training changes.");
            return;
        }
