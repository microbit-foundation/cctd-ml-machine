...
>
	...
	<div out:fly class="fixed bottom-0 w-full bg-white">
		<select class="absolute bg-white right-5 top-5 text-primarytext ml-2 mr-2 pl-2 pr-2" bind:value={$locale}>
			{#each locales as l}
				<option value={l}>{l}</option>
			{/each}
		</select>
		<div class="ml-auto mr-auto mt-5">
			<p class="text-center font-bold text-warning text-2xl ml-auto mr-auto">{$t("cookies.banner.title")}</p>
		</div>
		<div class="flex w-full mb-5">
			...
			<div class="flex w-2/5 flex-row mr-4 justify-center">
					<StandardButton onClick={acceptClicked}>{$t("cookies.banner.buttons.accept")}</StandardButton>
				<div class="mr-4"></div>
					<StandardButton
						color="warning"
