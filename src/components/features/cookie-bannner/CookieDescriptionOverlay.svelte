...
<script lang="ts">
  ...
  const cookies = [
    {
      name: 'ai_user',
      description: $tr('cookies.overlay.table.row.ai_user.description'),
    },
    {
      name: 'ai_session',
      description: $tr('cookies.overlay.table.row.ai_session.description'),
    },
    {
      name: 'cookie_consent',
      description: $tr('cookies.overlay.table.row.cookie_consent.description'),
    },
  ];
  ...
  <div class="w-150 h-150 p-3">
    <div>
      <p class="text-red-500 font-bold text-center text-xl">
        {$tr('cookies.overlay.title')}
      </p>
    </div>
    <div class="overflow-y-scroll h-10/11">
      <div class="p-2">
        <p class="font-bold">{$tr('cookies.overlay.question.whatAreCookies')}</p>
        <p>{$tr('cookies.overlay.answer.whatAreCookies')}</p>
        <p class="font-bold mt-2">{$tr('cookies.overlay.question.ourReasoning')}</p>
        <p>{$tr('cookies.overlay.answer.ourReasoning')}</p>
        <p class="font-bold mt-2">
          {$tr('cookies.overlay.question.storageDuration')}
        </p>
        <p>
          {$tr('cookies.overlay.answer.storageDuration')}
        </p>
        <p class="font-bold mt-2">
          {$tr('cookies.overlay.question.deleting')}
        </p>
        <p>
          {$tr('cookies.overlay.answer.deleting')}
        </p>
        <ul>
        ...
        <p class="font-bold mt-2">
          {$tr('cookies.overlay.question.consentChange')}
        </p>
        <p>
          {$tr('cookies.overlay.answer.consentChange')}
        </p>
      </div>
      <div>
        <p class="font-bold text-lg ml-2 mt-2">{$tr('cookies.overlay.table.title')}</p>
        <div class="flex">
            ...
            <div class="table-row">
              ...
              <div class="table-cell bg-gray-100">
                <p class="m-2 text-center font-bold">
                  {$tr('cookies.overlay.table.header.description')}
                </p>
              </div>
            </div>
