...
<script lang="ts">
  ...
  let gestureName = $gesture.getName();

  $: {
    if ($gesture.getName() !== gestureName) {
      gestureController.setGestureName($gesture.getID(), gestureName);
    }
  }
  ...
  function titleClicked(): void {
    if ($gesture.getName() === defaultNewName) {
      gestureController.setGestureName($gesture.getID(), '');
    }
  }
  ...
  function deleteRecording(recording: RecordingData) {
    ...
    gestureController.deleteRecording($gesture.getID(), recording.ID);
  }
  ...
  function onTitleKeypress(event: KeyboardEvent) {
    ...
    if (gestureName.length >= StaticConfiguration.gestureNameMaxLength) {
      event.preventDefault();
      alertUser(
  ...
  <div class="items-center flex relative">
    ...
    <GestureCard mr small>
      <div class="top-2 left-3 absolute flex flex-row justify-center items-center gap-4">
        <GestureDot gestureId={$gesture.getID()} />
    ...
    <GestureCard small mr elevated={$chosenGesture === $gesture.getID()}>
      {#if $chosenGesture !== $gesture.getID()}
    ...
    {#if $gesture.getRecordings().length > 0}
      <GestureCard small>
        <div class="flex p-2 h-30">
          {#each $gesture.getRecordings() as recording (String($gesture.getID()) + String(recording.ID))}
            <Recording
    ...
    {:else if $chosenGesture === $gesture.getID()}
      <GestureCard small>
