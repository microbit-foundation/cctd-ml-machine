...
<script lang="ts">
  ...
  import { currentPageComponent } from '../view/layout/currentComponentStore';

  async function getRoutedComponent(path: PathType) {
    switch (path) {
      case Paths.HOME:
        return (await import('../front-page/Homepage.svelte')).default;
      case Paths.VALIDATE:
        return (await import('../validation/ValidationPage.svelte')).default;
      case Paths.DATA:
        return (await import('../data/DataPage.svelte')).default;
      case Paths.TRAINING:
        return (await import('../training/TrainingPage.svelte')).default;
      case Paths.MODEL:
        return (await import('../output/ModelPage.svelte')).default;
      case Paths.FILTERS:
        return (await import('../filters/FilterPage.svelte')).default;
      case Paths.DEBUG:
        return (await import('../dev/DebugPage.svelte')).default;
      case Paths.MAKECODE:
        return (await import('../makecode/MakecodePage.svelte')).default;
    }
  }
