...
export const getModelTrainer = (modelEntry: ModelEntry, onTrainingIteration: (iteration: LossTrainingIteration) => void): ModelTrainer<MLModel> => {
    ...
    if (modelEntry.id === 'KNN') {
        ...
        const offset =
            currentAxis === Axes.X ? 0 : currentAxis === Axes.Y ? 1 : 2;
...
export const trainModel = (selectedOption: Writable<DropdownOption>, onTrainingIteration: (iteration: LossTrainingIteration) => void) => {
    const selectedModel = availableModels.find(model => model.id === get(selectedOption).id);
