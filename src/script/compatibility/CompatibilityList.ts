...
export const nonAllowedPlatforms = ["mobile"];
...
export class SemVerImpl implements SemVer {
	...
	private static compareSubVersions(subVersion1: number | string, subVersion2: number | string): number {
...
abstract class CompatibilityList {
	...
	public static getMinVersion(browserName: string): SemVer | undefined {
		return this.minVersions.find(version =>
			version.browser.toLowerCase() === browserName.toLowerCase())?.version;
	}
	...
	public static isVersionExempted(browserName: string,
																	version: SemVer,
																	operatingSystem: string): boolean {
	...
	public static isBrowserSupported(browserName: string): boolean {
		return this.minVersions.filter(value =>
			value.browser.toLowerCase() === browserName.toLowerCase()
		).length != 0;
	}
	...
	public static isSupported(browserName: string, version: SemVer, operatingSystem: string): boolean {
...
export class WebBluetoothCompatibility extends CompatibilityList {
	...
	public static isSupported(browserName: string,
														version: SemVer,
														operatingSystem: string): boolean {
