...
class Matrix<T> {
    ...
    public getRow(rowIndex: number): T[] {
        if (this.matrix.length <= rowIndex) {
            throw new Error(`There are not enough rows(${this.matrix.length}) to get row with index ${rowIndex}`)
        }
    ...
    public transposed(): Matrix<T> {
        return new Matrix(this.matrix[0].map((_, colIndex) => this.matrix.map(row => row[colIndex])));
    }
}
