...
class GestureConfidence implements Readable<ConfidenceData> {
    ...
    public subscribe(run: Subscriber<ConfidenceData>, invalidate?: ((value?: ConfidenceData | undefined) => void) | undefined): Unsubscriber {
    ...
    public setRequiredConfidence(value: number) {
        if (value < 0  || value > 1) {
            throw new Error("Could not set required confidence. Cannot go outside the 0.0-1.0 range");
        }
