...
export function persistantWritable<T>(key: string, initValue: T): Writable<T> {
	...
	if(initValue === null || initValue === undefined){
		throw new Error("Do not use 'null' or 'undefined' values as initial value. Later checking for changes, will result in stored changes being deleted")
	}
	...
	initValue = useStored ? (storedObject as {version: number, value: T}).value : initValue 
	const store: Writable<T> = writable(initValue);
	store.subscribe(val => localStorage.setItem(key, JSON.stringify({version: persistVersion, value: val})));
...
function areMatchingTypes(value1: unknown, value2: unknown): boolean{
	...
	if (value1 === null){ // This makes the type-checking incomplete, but otherwise we need to completely ban 'null' values in pesistant storage
		return true
	}
	...
	if (['boolean', 'number', 'string', 'null'].includes(type1)){ // value is a primitive
